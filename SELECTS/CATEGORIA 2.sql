...
/* Mostra el nom, l'edat, el partit i la província dels candidats que tenen menys de 25 anys.*/

SELECT CONCAT(p.nom, ' ', p.cognom1, ' ', p.cognom2) AS nom_complet, TIMESTAMPDIFF(YEAR,p.data_naixement,CURDATE()) AS edat,  c.nom_curt AS partit,  pr.nom AS provincia
	FROM candidats ca  
		INNER JOIN persones p ON ca.persona_id = p.persona_id 
		INNER JOIN candidatures c ON ca.candidatura_id = c.candidatura_id 
		INNER JOIN provincies pr ON ca.provincia_id = pr.provincia_id 
